...
class DeviceExceptionNewController extends ParentController
{
    public function actionIndex(){
        $params = \Yii::$app->request->getQueryParams();
        $query = DeviceExceptionNew::find()->joinWith(['device'])->joinWith(['user'])->where(['device.enable'=>1]);
...
//                }]);
        ...
        if(isset($params['uuid']) && !empty($params['uuid']))
            $query->andWhere(['like', 'device_exception_new.uuid', trim($params['uuid'])]);
        if(isset($params['treatment_result']))
            $query->andFilterWhere(['treatment_result'=>$params['treatment_result']]);
        if(isset($params['type']))
            $query->andFilterWhere(['device_exception_new.type'=>$params['type']]);
    ...
    public function actionTotal(){
        $params = \Yii::$app->request->getQueryParams();
        $query = DeviceExceptionNew::find()->select('COUNT(*) as total')->leftJoin('device','device_exception_new.uuid = device.uuid');
        $query->andWhere(['device_exception_new.type'=>1]);
        if(isset($params['date_from']) && isset($params['date_to']) && $params['date_from'] && $params['date_to']){
            $query->andWhere(['>=', 'device_exception_new.create_time', $params['date_from']]);
            $query->andWhere(['<', 'device_exception_new.create_time', $params['date_to']]);
        }
        ActionTool::addAddressFilter($query, $params);
        ActionTool::addGroup3UserIdFilter($query);
    ...
    public function actionTotalGroupByDistrict(){
        ...
        $query = DeviceExceptionNew::find()->select([$params['district_level'],'COUNT(*) as total'])->leftJoin('device','device_exception_new.uuid = device.uuid');
        $query->andWhere(['device_exception_new.type'=>1]);
        if(isset($params['date_from']) && isset($params['date_to']) && $params['date_from'] && $params['date_to']){
            $query->andWhere(['>=', 'device_exception_new.create_time', $params['date_from']]);
            $query->andWhere(['<', 'device_exception_new.create_time', $params['date_to']]);
    ...
    public function actionTotalGroupByMonthOfYear(){
        $params = \Yii::$app->request->getQueryParams();
        $query = DeviceExceptionNew::find()->select(['MONTH(device_exception_new.create_time) as month', 'COUNT(*) as total'])->leftJoin('device','device_exception_new.uuid = device.uuid');
        $query->andWhere(['device_exception_new.type'=>1]);
        $query->andWhere(['>=', 'device_exception_new.create_time', $params['year'].'-01-01']);
        $query->andWhere(['<', 'device_exception_new.create_time', ($params['year']+1).'-01-01']);
        ActionTool::addAddressFilter($query, $params);
        ActionTool::addGroup3UserIdFilter($query);
        $res = $query->asArray()->groupBy('MONTH(device_exception_new.create_time)')->all();
        $arr = [];
        foreach ($res as $item){
    ...
    public function actionClearAlarm(){
        ...
        $sql = 'UPDATE device_exception_new SET treatment_result = 2 WHERE uuid = :uuid AND treatment_result = 1';
        $res = Yii::$app->db->createCommand($sql, [':uuid'=>$uuid])->execute();
