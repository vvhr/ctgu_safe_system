...
use app\models\MinMaxLcDetail;
use Yii;
use yii\data\ActiveDataProvider;

class MinMaxLcDetailController extends ParentController
{
    public function actionIndex(){
        $params = \Yii::$app->request->getQueryParams();
        $query = MinMaxLcDetail::find()->joinWith('device')->joinWith('user')->with('homeUser');
        ...
        if(isset($params['imei']))
            $query->andFilterWhere(['min_max_lc_detail.imei'=>$params['imei']]);
...
//        return $query->createCommand()->getRawSql();
     ...
     */
    public function actionCreateMinMaxLcDetailByDate(){
        $params = Yii::$app->request->getBodyParams();
        $date = $params['date'];
        $res = MinMaxLcDetail::getAndInsertMaxMinLCRelatedRecordsByDate($date);
        return $res;
    }
}
